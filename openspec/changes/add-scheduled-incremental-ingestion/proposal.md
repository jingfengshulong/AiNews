...
- Make startup ingestion default to a bounded 24-hour catch-up window, so a fresh server can populate recent news without attempting unbounded history.
- Make scheduled runs incremental by only adding source items that are new since the last successful source fetch, while preserving existing raw-item, article, and signal deduplication.
...
- Affects source adapters where publication-time or external-id filtering must be applied after fetching source windows.
